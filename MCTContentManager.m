...
-(void) initEvents {
    ...
    MCTEvent *event0= [[MCTEvent alloc] init]; event0.objectID = 0;	event0.name = @"Taco Tuesday Happy Hour";	event0.details = @"Fellow Celiac's and gluten-free friends come enjoy a fun, festive, and oh did I forget to mention tasty, happy hour at one of my favorite gluten-free spots in PA! I can't wait to meetchew(all)there ;) ";	event0.date = [	12/12/16	5:30 PM	]	event0.admin = _users[13];	event0.isGoing = NO;	event0.capacity = 7;	event0.restaurant = _restaurants[6];	event0.dietTags = @[ _dietTags[6], _dietTags[7]];	event0.guests = [[NSMutableArray alloc] initWithArray: @[_users[13]]];	[_events addObject:event0];
    MCTEvent *event1= [[MCTEvent alloc] init]; event1.objectID = 1;	event1.name = @"Teagan's Vegan Extravaganza";	event1.details = @"Hi! I'm a vegan and a total foodie. I am new to the area and I'm super excited to explore the vegan food scene and I want to share my food adventures with you! Come join me as I test out Calafia Cafe - I've heard only great things, they even have an entire vegan/veggie friendly menu!! ";	event1.date = [	12/10/16	6:00 PM	]	event1.admin = _users[18];	event1.isGoing = NO;	event1.capacity = 12;	event1.restaurant = _restaurants[0];	event1.dietTags = @[_dietTags[0], _dietTags[1], _dietTags[2], _dietTags[7]];	event1.guests = [[NSMutableArray alloc] initWithArray: @[_users[18], _users[27]]];	[_events addObject:event1];
    MCTEvent *event2= [[MCTEvent alloc] init]; event2.objectID = 2;	event2.name = @"No Nuts Allowed";	event2.details = @"If you're like me then you find yourself constantly wondering why all the best baked goods have nuts? Come meet me at Shuly's to discuss this and other #nutallergypeopleproblems while we enjoy some of the best nut free baked goods I have ever had (sorry mom)";	event2.date = [	12/10/16	10:00 AM	]	event2.admin = _users[21];	event2.isGoing = NO;	event2.capacity = 7;	event2.restaurant = _restaurants[2];	event2.dietTags = @[_dietTags[2], _dietTags[3], _dietTags[6], _dietTags[7]];	event2.guests = [[NSMutableArray alloc] initWithArray: @[_users[21], _users[10], _users[78]]];	[_events addObject:event2];
    MCTEvent *event3= [[MCTEvent alloc] init]; event3.objectID = 3;	event3.name = @"No Dairy With Larry";	event3.details = @"Hi, I'm Larry and I can't eat dairy. If you're like me and can't eat dairy, or your name rhymes with dairy, come join me for a nice weekend lunch of some of the area's best falafel and hummus (and did I mention it's all dairy free)!";	event3.date = [	12/11/16	12:30 PM	]	event3.admin = _users[60];	event3.isGoing = NO;	event3.capacity = 8;	event3.restaurant = _restaurants[14];	event3.dietTags = @[_dietTags[0], _dietTags[1]_dietTags[7]];	event3.guests = [[NSMutableArray alloc] initWithArray: @[_users[60], _users[57], _users[59]]];	[_events addObject:event3];
    MCTEvent *event4= [[MCTEvent alloc] init]; event4.objectID = 4;	event4.name = @"Bob's Brunch Bunch";	event4.details = @"Vegetarian Brunch - enough said. meetchewthere Saturday Dec 11 @ 11";	event4.date = [	12/11/16	11:00 AM	]	event4.admin = _users[38];	event4.isGoing = NO;	event4.capacity = 5;	event4.restaurant = _restaurants[47];	event4.dietTags = @[_dietTags[0]_dietTags[2], _dietTags[7]];	event4.guests = [[NSMutableArray alloc] initWithArray: @[_users[38]]];	[_events addObject:event4];
    MCTEvent *event5= [[MCTEvent alloc] init]; event5.objectID = 5;	event5.name = @"Not Your Mom's BBQ";	event5.details = @"I've searched high and low and guys I finally found it - a dairy free, gluten free bbq joint in the bay area!!! If you're like me and salivating just thinking about all that smokey roasted bbq goodness meet me at this hidden gem - aka the most underated bbq joint in town!";	event5.date = [	12/16/16	6:00 PM	]	event5.admin = _users[32];	event5.isGoing = NO;	event5.capacity = 12;	event5.restaurant = _restaurants[31];	event5.dietTags = @[_dietTags[2], _dietTags[7]];	event5.guests = [[NSMutableArray alloc] initWithArray: @[_users[32], _users[64], _users[82], _users[3]]];	[_events addObject:event5];
    MCTEvent *event6= [[MCTEvent alloc] init]; event6.objectID = 6;	event6.name = @"Beginning To Be A Vegan";	event6.details = @"Calling all vegans. I recently adopted a completely vegan lifestyle and I am looking for fellow vegans to eat and share/explore recipes and restaurants with. I can't wait to meetchewthere :)";	event6.date = [	12/17/16	5:30 PM	]	event6.admin = _users[30];	event6.isGoing = NO;	event6.capacity = 11;	event6.restaurant = _restaurants[32];	event6.dietTags = @[_dietTags[2], _dietTags[7]];	event6.guests = [[NSMutableArray alloc] initWithArray: @[_users[30]]];	[_events addObject:event6];
    MCTEvent *event7= [[MCTEvent alloc] init]; event7.objectID = 7;	event7.name = @"Coffee-Break w/ Kate";	event7.details = @"Its Monday and we're all still wishing it was the weekend, so take a break from work and join me for some morning pastries and coffee at Zest Bakery. Their stuff is dariy free, gluten free and 100% delicious :D";	event7.date = [	12/12/16	10:00 AM	]	event7.admin = _users[5];	event7.isGoing = NO;	event7.capacity = 9;	event7.restaurant = _restaurants[34];	event7.dietTags = @[_dietTags[3], _dietTags[6], _dietTags[7]];	event7.guests = [[NSMutableArray alloc] initWithArray: @[_users[5], _users[8], _users[9]]];	[_events addObject:event7];
    MCTEvent *event8= [[MCTEvent alloc] init]; event8.objectID = 8;	event8.name = @"No Eggs, No Problem";	event8.details = @"To all my egg free Italians. Are you looking for the perfect eggless and authentic pasta dishes, well come dine with me at Il Fornaio - it's pasta just like my Grandma used to make (minus the eggs)!";	event8.date = [	12/16/16	7:00 PM	]	event8.admin = _users[26];	event8.isGoing = NO;	event8.capacity = 5;	event8.restaurant = _restaurants[36];	event8.dietTags = @[_dietTags[4], _dietTags[6]];	event8.guests = [[NSMutableArray alloc] initWithArray: @[_users[26]]];	[_events addObject:event8];
    MCTEvent *event9= [[MCTEvent alloc] init]; event9.objectID = 9;	event9.name = @"Tour of (Gluten Free) Brazil";	event9.details = @"It may be Thursday, but who cares! Come celebrate the weekend early and see how Brazil does gluten free.";	event9.date = [	12/15/16	6:00 PM	]	event9.admin = _users[85];	event9.isGoing = NO;	event9.capacity = 11;	event9.restaurant = _restaurants[39];	event9.dietTags = @[_dietTags[2], _dietTags[3]];	event9.guests = [[NSMutableArray alloc] initWithArray: @[_users[85], _users[84], _users[86]]];	[_events addObject:event9];
    MCTEvent *event10= [[MCTEvent alloc] init]; event10.objectID = 10;	event10.name = @"Hump Day Happy Hour";	event10.details = @"Any one else feeling a little slow and in a rut on hump day? Mix it up and come out to happy hour at Sam's - the fish place where you eat a totaly SHELLFISH FREE meal.";	event10.date = [	12/14/16	5:30 PM	]	event10.admin = _users[47];	event10.isGoing = NO;	event10.capacity = 9;	event10.restaurant = _restaurants[35];	event10.dietTags = @[_dietTags[3]_dietTags[7]];	event10.guests = [[NSMutableArray alloc] initWithArray: @[_users[47], _users[51]]];	[_events addObject:event10];
    MCTEvent *event11= [[MCTEvent alloc] init]; event11.objectID = 11;	event11.name = @"Bowling for Breakfast";	event11.details = @"Hey hey early birds. If you like bowls and breakfast, come meet some fellow dairy free friends and join our pre-work breakfast hangout at the best smoothie/acai bowl/juice bar/whaterever you want to call it place in town!";	event11.date = [	12/14/16	7:30 AM	]	event11.admin = _users[79];	event11.isGoing = NO;	event11.capacity = 7;	event11.restaurant = _restaurants[30];	event11.dietTags = @[_dietTags[0], _dietTags[1], _dietTags[2], _dietTags[7]];	event11.guests = [[NSMutableArray alloc] initWithArray: @[_users[79]]];	[_events addObject:event11];
    MCTEvent *event12= [[MCTEvent alloc] init]; event12.objectID = 12;	event12.name = @"Mo Meat, Mo Problems";	event12.details = @"Love Indian food? Don't love meat? Love any excuse to leave the office on a Monday? Well so do we so hurry up now and RSVP.";	event12.date = [	12/12/16	12:00 PM	]	event12.admin = _users[49];	event12.isGoing = NO;	event12.capacity = 6;	event12.restaurant = _restaurants[15];	event12.dietTags = @[_dietTags[0], _dietTags[1], _dietTags[2], _dietTags[7]];	event12.guests = [[NSMutableArray alloc] initWithArray: @[_users[49], _users[48]]];	[_events addObject:event12];
    MCTEvent *event13= [[MCTEvent alloc] init]; event13.objectID = 13;	event13.name = @"Not About the Nuts";	event13.details = @"Fellow foodies with nut allergies, if you're like me and all about food, but not about the nuts, come check out Lyfe Kitchen with me! This place is great - they know exactly what goes into every dish and super careful not to cross-contaminate. This will be my third time back in 2 weeks (unless I can't resist the gourmet goodness and return between now and then haha).";	event13.date = [	12/15/16	8:00 AM	]	event13.admin = _users[46];	event13.isGoing = NO;	event13.capacity = 7;	event13.restaurant = _restaurants[3];	event13.dietTags = @[_dietTags[1], _dietTags[6], _dietTags[7]];	event13.guests = [[NSMutableArray alloc] initWithArray: @[_users[46], _users[47], _users[67]]];	[_events addObject:event13];
    MCTEvent *event14= [[MCTEvent alloc] init]; event14.objectID = 14;	event14.name = @"Soy What? Lunch @ The Loving Hut";	event14.details = @"Do you love food, but can't eat soy? Well me too and I'd love to meetchew for lunch at the Loving Hut :)";	event14.date = [	12/16/16	12:30 PM	]	event14.admin = _users[75];	event14.isGoing = NO;	event14.capacity = 5;	event14.restaurant = _restaurants[44];	event14.dietTags = @[_dietTags[0], _dietTags[1], _dietTags[2], _dietTags[7]];	event14.guests = [[NSMutableArray alloc] initWithArray: @[_users[75]]];	[_events addObject:event14];}
